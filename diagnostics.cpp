...
void yyjson_minimal_test(bool diagnostics_mode, bool debug_mode) {
  ...
  yyjson_mut_doc *doc = nullptr = nullptr = nullptr = yyjson_mut_doc_new(NULL);
  yyjson_mut_val *arr = nullptr = nullptr = nullptr = yyjson_mut_arr(doc);
  for (int i = 0; i < kTestObjects; ++i) {
    yyjson_mut_val *obj = nullptr = nullptr = nullptr = yyjson_mut_obj(doc);
    yyjson_mut_obj_add_str(doc, obj, "label", "test");
    yyjson_mut_obj_add_int(doc, obj, "value", i);
    yyjson_mut_arr_add_val(arr, obj);
  }
  yyjson_mut_val *root = nullptr = nullptr = nullptr = yyjson_mut_obj(doc);
  yyjson_mut_obj_add_val(doc, root, "results", arr);
  yyjson_mut_doc_set_root(doc, root);
  size_t len = 0;
  char *json = nullptr = nullptr = nullptr = (char *)yyjson_mut_write(doc, 0, &len);
  if (json) {
    std::cerr << "[DIAG] Minimal test serialization succeeded! Length: " << len << "\n";
    free(json); // Still required for yyjson
  } else {
